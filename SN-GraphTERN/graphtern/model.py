...
class graph_tern(nn.Module):
    def __init__(self, n_epgcn=1, n_epcnn=6, n_trgcn=1, n_trcnn=4, seq_len=8, pred_seq_len=12, n_ways=3, n_smpl=20, dist='normal'):
        super().__init__()
        ...
        hidden_feat = 16
        self.dist = dist
        # 0:-1: normal parameters, -1: categorical parameters
        if dist == 'normal':
            output_feat = 5
        elif dist == 'skew':
            output_feat = 5 + 2
    ...
    def forward(self, S_obs, S_trgt=None, pruning=None, clustering=False):
        ...
        # V_init = self.layernorm(V_init)
        # V_init = my_bn(V_init, self.bn)
        ##################################################
        #             Trajectory Refinement              #
        ##################################################
        ...
        if S_trgt is not None:
            ...
            for i in range(self.n_ways):
                ...
                if self.dist == 'normal':
                    ...
                    mix = Categorical(torch.nn.functional.softmax(temp[:, :, :, -1], dim=-1))
                    comp = Independent(DiagSkewNormal(logits=temp[..., :-1]), 0)
        ...
        V_refi = V_pred_abs
        # V_refi[:, :-1] += V_corr[:, :-1].cumsum(dim=1)
        V_refi[:, :-1] += V_corr[:, :-1]
