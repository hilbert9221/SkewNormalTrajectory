...
checkpoint_dir = f'./checkpoint/{test_args.date}_{test_args.dist}_graph-tern_{test_args.dataset}/'
if len(test_args.date) == 0:
    test_args.date = strftime("%y-%m-%d_%H-%M-%S", gmtime())
my_log = create_logger(__name__, silent=False, to_disk=False, log_file=f'{checkpoint_dir}/log.txt')
...
def test(KSTEPS=20):
    ...
    for batch_idx, batch in enumerate(test_loader):
        ...
        # V_init, V_pred, V_refi, valid_mask = model(S_obs, clustering=True)
