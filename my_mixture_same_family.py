...
def my_logsumexp(x, eps=1e-20, dim=-1):
    M = x.max(dim=dim, keepdim=True)[0]
    return M + (x - M).exp().sum(dim=dim, keepdim=True).clamp(min=eps).log()
...
class MixtureSameFamily(Distribution):
    ...
    def pdf(self, x):
        x = self._pad(x)
        try:
            pdf_x = self.component_distribution.pdf(x)
        except:
            pdf_x = self.component_distribution.base_dist.pdf(x)
        mix_prob = self.mixture_distribution.probs

        return torch.sum(pdf_x * mix_prob, dim=-1)

    def direct_log_prob(self, x):
        if self._validate_args:
            self._validate_sample(x)
        x = self._pad(x)
        prob_x = self.component_distribution.base_dist.pdf(x)  # [S, B, k]
        mix_prob = self.mixture_distribution.probs  # [B, k]
        nll = - (prob_x * mix_prob).sum(-1).clamp(min=1e-20).log()
        return nll
    ...
    def log_prob(self, x):
        ...
        log_mix_prob = torch.log_softmax(self.mixture_distribution.logits,
                                         dim=-1)  # [B, k]
        return my_logsumexp(log_prob_x + log_mix_prob, dim=-1)  # [S, B]
    ...
    def uncertainty(self,):
        ...
        comp = self.component_distribution.base_dist
        cov = comp.variance
        if (cov.dim() - probs.dim()) == 1:
            cov = torch.diag_embed(cov)
        # cov_norm = torch.linalg.matrix_norm(cov, ord='fro')
        # sum of diagonal elements
        cov_norm = cov.diagonal(dim1=-2, dim2=-1).sum(-1)
        weighted_norm = (probs * cov_norm).sum(-1)

        entropy = comp.entropy()
        if entropy.dim() > probs.dim():
            entropy = entropy.sum(-1)
        weighted_entropy = (probs * entropy).sum(-1) - (probs * probs.clamp(min=1e-20).log()).sum(-1)
        return weighted_norm, weighted_entropy

    def diversity(self,):
        probs = self._pad_mixture_dimensions(self.mixture_distribution.probs).unsqueeze(-1)
        comp = self.component_distribution.base_dist
        cov = comp.variance
        if cov.dim() < probs.dim():
            cov = torch.diag_embed(cov)
        mean_cond_var = (probs * cov).sum(dim=-3)
        # delta: [..., 1, dim]
        delta = self.mean[..., None, :] - comp.mean
        delta = delta.unsqueeze(-1)
        var_cond_mean = (probs * (delta @ delta.transpose(-1, -2))).sum(dim=-3)
        cov_mix = mean_cond_var + var_cond_mean
        # return torch.linalg.matrix_norm(cov_mix, ord='nuc')
        return cov_mix.diagonal(dim1=-2, dim2=-1).sum(-1)
    ...
    def cov(self,):
        ...
        if cov.dim() < probs.dim():
            cov = torch.diag_embed(cov)
        mean_cond_var = (probs * cov).sum(dim=-3)
        return torch.linalg.matrix_norm(mean_cond_var)
